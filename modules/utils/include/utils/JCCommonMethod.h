...
// #define ARC_TO_RAD  0.0174532925199433
...
inline bool isPot(int size)
{
    return (size & (size - 1)) == 0;
}
template <class Container> void split(const std::string &str, Container &cont, const std::string &delims = " ")
{
...
// 基础模板
template <typename T> class ToString
{
  public:
...
inline std::string wideToUtf8(const std::wstring &wideText)
{
    ...
    const int length = ::WideCharToMultiByte(CP_UTF8, 0, wideText.data(), (int)wideText.size(), narrowString,
