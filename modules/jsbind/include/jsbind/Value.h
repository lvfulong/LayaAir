...
inline jsvm_value makeStringUtf8(const char* value, size_t length)
{
    GET_ENV
    jsvm_value result;
    jsvm_status status;
    status = jsvm_create_string_utf8(env, value, length, &result);
    DEBUG_CHECK(status == jsvm_status::jsvm_ok);
    return result;
}
...
inline jsvm_value makeStringUtf16(const char16_t* value, size_t length)
{
    GET_ENV
    jsvm_value result;
    jsvm_status status;
    status = jsvm_create_string_utf16(env, value, length, &result);
    DEBUG_CHECK(status == jsvm_status::jsvm_ok);
    return result;
}
