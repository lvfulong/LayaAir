...
#define FALSE_RET0(v,msg) {if(!(v)){ LOGE(msg); return false;}}  
	...
	void JCServerFileCache::setCachePath( const char* p_pszCachePath ){
		...
		//try{
			...
			if( !fs::exists(m_strCachePath.c_str(), error)){
				//如果目录不存在
				if (!fs::create_directories(m_strCachePath.c_str(), error))
				{
					LOGE("create_directories %s failed", m_strCachePath.c_str());
				}
			}
		//}catch(...){
		//	if( global_onCreateFileError){
	...
	void JCServerFileCache::switchToApp(const char* p_pszWebBase){
		...
		//try {
			std::error_code error;
			if (!fs::exists(m_strCacheFilesPath.c_str(), error)) {
				//如果目录不存在				
				if (!fs::create_directories(m_strCacheFilesPath.c_str(), error))
				{
					LOGE("create_directories %s failed", m_strCacheFilesPath.c_str());
				}
			}
		//}
		//catch (...) {
		...
		//try { 
			if (!fs::exists(sourceidPath.c_str(), error)) {
				if (!fs::create_directories(sourceidPath.c_str(), error))
				{
					LOGE("create_directories %s failed", sourceidPath.c_str());
				}
			}
		//}
		//catch (...) {
    ...
    */
    unsigned int JCServerFileCache::getFileID(const char* Url) {
        ...
        if (m_pFuncTransUrl && m_pFuncTransUrlData) {
            ...
            nUrl = (*m_pFuncTransUrl)(m_pFuncTransUrlData,Url).c_str();
            if (strlen(nUrl) == 0)
                nUrl = (char*)Url;
	...
	std::string JCCachedFileSys::fileToPath(typeFile p_nFile, std::string& p_strPath, bool p_bCreateDir) {
		...
		if (p_bCreateDir) {
			...
			if (!hasDir(dirID)) {
				std::error_code error;
				if (!fs::exists(p_strPath.c_str(), error)) {
					//如果目录不存在
					fs::create_directories(p_strPath.c_str(), error);
					setHasDir(dirID);
				}
				else {
					setHasDir(dirID);
