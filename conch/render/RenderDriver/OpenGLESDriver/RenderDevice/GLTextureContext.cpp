...
namespace laya
{
	static void TexImage2DSafe(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const void *pixels)
	{
		unsigned bytesPerComponent, componentsPerPixel;
		if (pixels == nullptr && JCLayaGL::computeFormatAndTypeParameters(format, type, &componentsPerPixel, &bytesPerComponent))
		{
	...
	GLTextureContext::GLTextureContext(GLESEngine* engine): GLObject(engine)
	{
		...
		m_compressdTextureS3tc_srgb = (CompressdTextureS3tc_srgbExt*)m_engine->getExtension(WebGLExtension::WEBGL_compressed_texture_s3tc_srgb);
		m_compressedTextureEtc1 = (CompressedTextureEtc1Ext*)m_engine->getExtension(WebGLExtension::WEBGL_compressed_texture_etc1);
		m_compressedTextureS3tc = (CompressedTextureS3tcExt*)m_engine->getExtension(WebGLExtension::WEBGL_compressed_texture_s3tc);
		m_compressedTextureETC = (CompressedTextureETCExt*)m_engine->getExtension(WebGLExtension::WEBGL_compressed_texture_etc);
		m_compressedTextureASTC = (CompressedTextureASTCExt*)m_engine->getExtension(WebGLExtension::WEBGL_compressed_texture_astc);
	...
	int GLTextureContext::getGLRTTexMemory(int width, int height, RenderTargetFormat colorFormat, RenderTargetFormat depthStencilFormat, bool generateMipmap, int multiSamples, bool cube)
	{
 ...
 //setTextureSubImageData TODO
    ...
    void GLTextureContext::setTextureDDSData(GLESInternalTex *texture, const DDSTextureInfo &ddsInfo)
    {
        ...
        for (int index = 0; index < mipmapCount; index++) {
            int32_t imageSize = std::max(4, mipmapWidth) / 4 * std::max(4, mipmapWidth) / 4 * blocksBytes;
