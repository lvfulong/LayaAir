...
namespace laya
{
	...
	void GLCapable::initCapable(bool isWebgl2)
	{
		...
		if (isWebgl2)
		{
			value = !!getExtension(WebGLExtension::EXT_color_buffer_float);
		}
		else 
		{
			value = (!!getExtension(WebGLExtension::OES_texture_half_float)) && (!!getExtension(WebGLExtension::OES_texture_half_float_linear));
		}
		...
		m_capabilityMap[RenderCapable::GRAPHICS_API_GLES3] = value;
