...
@interface postCmdEditBoxWXParam : NSObject
{
@public
    ...
    std::string m_prompt;
    std::string m_promptColor;
    std::string m_inputType;
}
-(id)init:(const char*)defaultValue maxLength:(int)maxLength multiple:(bool)multiple confirmHold:(bool)confirmHold confirmType:(const char*)confirmType propmt:(const char*)prompt promptColor:(const char*)propmtColor inputType:(const char*)inputType;
@end
...
@interface postCmdInt2StringParam : NSObject
{
@public
    int iParam0;
    std::string sBuffer0;
