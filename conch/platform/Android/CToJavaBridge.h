...
public:
    ...
    */
	...
	static void getJavaByteArray(jbyteArray jba, JNIEnv *env, char **ppData, int& len);	
	...
	bool callMethod(const char* className, const char* methodName, std::string s1, JavaRet& ret);
	...
	bool callMethod(const char* className, const char* methodName, int n, const char* s1, const char* s2, JavaRet& ret);
	...
	bool getClassAndStaticMethod(const char* className, const char* methodName, const char* methodSign, JNIEnv** ppEnv, jclass* pClazz, jmethodID* pMethodId);

	bool getSafeInsetRect(int& left, int& top, int& right, int& bottom);
...
public:
   ...
   // JavaMethodMap		                m_StaticMethods;
	...
	jmethodID							m_jMethodGetDisplaySafeArea;
