...
namespace laya
{
    ...
    */
    ...
    {
    public:
        JCLayaGL(int nWidth, int nHeight, JCImageManager* pImageManager, JCIDGenerator* pIDGenerator, JCIDGenerator* pProgramLocationTable);
		...
		virtual void bindBufferBase(GLenum target, GLuint index, WebGLBuffer buffer);
		virtual void bindBufferRange(GLenum target, GLuint index, WebGLBuffer buffer, GLintptr offset, GLsizeiptr size);
        ...
        void getActiveUniform(WebGLActiveInfo* out, const std::string& vs, const std::string& ps, const std::string& define, int nIndex);
        void getActiveAttrib(WebGLActiveInfo* out, const std::string& vs, const std::string& ps, const std::string& define, int nIndex);
        int getAttribLocation(const std::string& vs, const std::string& ps, const std::string& define, const std::string& name);
        void getProgramInfoLog(std::string& out, const std::string& vs, const std::string& ps, const std::string& define);
        const char* getActiveUniformBlockName(int program, int uniformBlockIndex);
        int getUniformBlockIndex(int program, const std::string& uniformBlockName);
        void uniformBlockBinding(int program, int uniformBlockIndex, int uniformBlockBinding);
        void getFramebufferAttachmentParameter(int& result, int target, int attachment, int name);
        void getShaderInfoLog(std::string& out, const std::string& source,int type);
        void getUniform(std::vector<float>& out, const std::string& locationName);
        void getBooleanv(GLboolean& result, int name);
        void readPixels(std::vector<uint8_t>& out, int x, int y, int width, int height, int format, int type);
        void getFloatv(GLfloat& result, int name);
        void getIntegerArrayv(std::vector<int>& out, int name);
        void getFloatArrayv(std::vector<float>& out, int name);
        void getShaderPrecisionFormat(int shaderType, int precisionType, GLint* range, GLint* precision);
        GLboolean isEnabled(GLenum cap);
        void getVertexAttribfv(std::vector<float>& out, int index, int target);
        void flushCommand();
    public:
    ...
    public:
		...
		int getRealID(GLuint fakeID);
    private:
        void _getFloatArrayv(std::vector<float>& out, int name);
        void _getIntegerArrayv(std::vector<int>& out, int name);
        void _readPixels(std::vector<uint8_t>& out, int x, int y, int width, int height, int format, int type);
        void _getShaderInfoLog(std::string& out, const std::string& source,int type);
    ...
    private:
        ...
        std::string                         m_string;
        int                                 m_int;
        JCGpuProgramTemplate*               m_pShaderTemplate = nullptr;
        JCGpuProgram*                       m_pGpuProgram = nullptr;
        std::string                         m_sShaderVS;
        std::string                         m_sShaderPS;
        std::string                         m_sShaderDefine;
        char                                m_uniformBlockName[256] = { 0 };
    };
}
//------------------------------------------------------------------------------
