...
namespace laya
{
    ...
    JCLayaGL::JCLayaGL(int nWidth, int nHeight,JCImageManager* pImageManager,JCIDGenerator* pIDGenerator, JCIDGenerator* pProgramLocationTable)
    {
	...
	void JCLayaGL::bindBufferBase(GLenum target, GLuint index, WebGLBuffer buffer)
	{
		int realID = (buffer == 0) ? 0 : m_pIDGenerator->getRealID(buffer);
		glBindBufferBase(target, index, realID);
#ifdef DEBUG_WEBGL
		LOGI("bindBufferBase target=%d,index=%d,realBuffer=%d,fakeBuffer=%d", target, index, realID, buffer);
		getError();
#endif
	}
	void JCLayaGL::bindBufferRange(GLenum target, GLuint index, WebGLBuffer buffer, GLintptr offset, GLsizeiptr size)
	{
		int realID = (buffer == 0) ? 0 : m_pIDGenerator->getRealID(buffer);
		glBindBufferRange(target, index, realID, offset, size);

#ifdef DEBUG_WEBGL
		LOGI("bindBufferRange target=%d,index=%d,realBuffer=%d,fakeBuffer=%d,offset=%d,size=%d", target, index, realID, buffer, offset, size);
		getError();
#endif
	}
    void JCLayaGL::bindFramebuffer(GLenum target, WebGLFramebuffer framebuffer)
    {
        int realID = (framebuffer == 0) ? g_nMainFrameBuffer : m_pIDGenerator->getRealID(framebuffer);
	...
	void JCLayaGL::clearBufferfi(GLenum buffer, GLint drawbuffer, GLfloat depth, GLint stencil)
	{
		::glClearBufferfi(buffer, drawbuffer, depth, stencil);
#ifdef DEBUG_WEBGL
		LOGI("clearBufferfi buffer=%d drawbuffer=%d depth %f stencil %d", buffer, drawbuffer, depth, stencil);
		getError();
#endif
	}
    ...
    void JCLayaGL::scissor(GLint x, GLint y, GLsizei width, GLsizei height)
    {
        ...
        /*if (nFrameBuffer == g_nMainFrameBuffer)
        {
	...
	int JCLayaGL::getRealID(GLuint fakeID)
	{
		int realID = m_pIDGenerator->getRealID(fakeID);
		return realID;
	}
...
//#endif
...
#endif
        //webgl标准依据？
        JCImage::flipY(GL_UNSIGNED_BYTE, GL_RGBA, width, height, cbuffer.data());
	...
	bool JCLayaGL::computeFormatAndTypeParameters(GLenum format, GLenum type, unsigned* componentsPerPixel, unsigned* bytesPerComponent)
	{
		switch (format) 
		{
		case GL_ALPHA:
    ...
    //
    const char* JCLayaGL::getString(unsigned int name) {
        m_string = JCConch::s_pConchRender->postTaskFromJSToRenderSync([name]()->const char* {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            return(const char*)glGetString(name);
        }).get();
        return m_string.c_str();
    }
    int JCLayaGL::getIntegerv(int name) {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([name]()->int {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            int result;
    ...
    int JCLayaGL::getParameter(int name) {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([name]()->int {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            int result;
            ::glGetIntegerv(name, &result);
    ...
    int JCLayaGL::getShaderParameter(const char* src,int type,int name) {
        ...
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([name, type, &source]()->int {
            ...
            ::glGetShaderiv(nShader, name, &result);
            glDeleteShader(nShader);
            return result;
        }).get();
    }
    int JCLayaGL::getProgramParameter(const char* vs, const char* ps, const char* define, int type)
    {
        std::string str_vs = vs;
        std::string str_ps = ps;
        std::string str_define = define;
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([&str_vs, &str_ps, &str_define, type, this]()->int {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            int result;
            _createShader(str_vs, str_ps, str_define);
            ::glGetProgramiv(m_pGpuProgram->getGpuProgram(), type, &result);
            return result;
        }).get();
    }
    void JCLayaGL::getActiveAttrib(WebGLActiveInfo* out, const std::string& vs, const std::string& ps, const std::string& define, int nIndex)
    {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&vs, &ps, &define, this, out, nIndex]()->bool {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            _createShader(vs, ps, define);
            ::glGetActiveAttrib(m_pGpuProgram->getGpuProgram(), nIndex, out->bufsize, &out->length, &out->size, &out->type, out->name);
            return true;
        }).get();
    }
    void JCLayaGL::getActiveUniform(WebGLActiveInfo* out, const std::string& vs, const std::string& ps, const std::string& define, int nIndex) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&vs, &ps, &define, this, out, nIndex]()->bool {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            _createShader(vs, ps, define);
            ::glGetActiveUniform(m_pGpuProgram->getGpuProgram(), nIndex, out->bufsize, &out->length, &out->size, &out->type, out->name);
            return true;
        }).get();
    }
    int JCLayaGL::getAttribLocation(const std::string& vs, const std::string& ps, const std::string& define, const std::string& name) {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([&vs, &ps, &define, this, &name]()->int {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            _createShader(vs, ps, define);
            return ::glGetAttribLocation(m_pGpuProgram->getGpuProgram(), name.c_str());
        }).get();
    }
    const char* JCLayaGL::getActiveUniformBlockName(int program, int uniformBlockIndex) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([program, uniformBlockIndex, this]()->bool {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            GLsizei nLength = 0;
            glGetActiveUniformBlockName(this->getRealID(program), uniformBlockIndex, 256, &nLength, m_uniformBlockName);
            return true;
        }).get();
        return m_uniformBlockName;
    }
    int JCLayaGL::getUniformBlockIndex(int program, const std::string& uniformBlockName) {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([program, this, &uniformBlockName]()->int {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            return glGetUniformBlockIndex(this->getRealID(program), uniformBlockName.c_str());
        }).get();
    }
    void JCLayaGL::uniformBlockBinding(int program, int uniformBlockIndex, int uniformBlockBinding) {
    //TODO delete
        JCConch::s_pConchRender->postTaskFromJSToRenderAsync([program, uniformBlockIndex, uniformBlockBinding, this]() {
            glUniformBlockBinding(JCConch::s_pConchRender->m_pLayaGL->getRealID(program), uniformBlockIndex, uniformBlockBinding);
        });
    }
    void JCLayaGL::getProgramInfoLog(std::string& out, const std::string& vs, const std::string& ps, const std::string& define)
    {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&vs, &ps, &define, this, &out]()->bool {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            _createShader(vs, ps, define);
            GLint nInfoLen = 0;
    ...
    void JCLayaGL::_createShader(const std::string& vs, const std::string& ps, const std::string& define)
    {
    ...
    void JCLayaGL::getUniform(std::vector<float>& out, const std::string& locationName) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([this, &out, &locationName]()->bool {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            _getUniform(out, locationName);
            return true;
        }).get();
    }
    void JCLayaGL::_getUniform(std::vector<float>& out, const std::string& locationName) {
        static int iRet[16];
        ...
        else
        {
            ...
            out.push_back((float)(typeDesc.category));
            ...
            else
            {
                ...
                for (int i = 0; i < typeDesc.size; i++)
                {
                    out.push_back((float)iRet[i]);
                }
            }
        }
    }
    void JCLayaGL::flushCommand() {
    ...
    void JCLayaGL::getFramebufferAttachmentParameter(int& result, int target, int attachment, int name) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&result, target, attachment, name]()->bool {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            ::glGetFramebufferAttachmentParameteriv(target, attachment, name, &result);
            return true;
        }).get();
    }
    void JCLayaGL::getShaderInfoLog(std::string& out, const std::string& source,int type) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&source, type, this, &out]()->bool {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            _getShaderInfoLog(out, source, type);
            return true;
        }).get();
    }
    void JCLayaGL::_getShaderInfoLog(std::string& out, const std::string& source,int type) {
        GLuint nShader = glCreateShader(type);
        const char* strShaders[1] = {source.c_str()};
        glShaderSource(nShader, 1, strShaders, NULL);
        glCompileShader(nShader);
    ...
    void JCLayaGL::getBooleanv(GLboolean& result, int name) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&result, name]()->bool {
            if (name == GL_BLEND || name == GL_CULL_FACE ||
                name == GL_DEPTH_TEST || name == GL_DEPTH_WRITEMASK ||
                name == GL_DITHER || name == GL_SAMPLE_COVERAGE_INVERT ||
                name == GL_SCISSOR_TEST || name == GL_STENCIL_TEST)
            {
                JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            }
    ...
    void JCLayaGL::readPixels(std::vector<uint8_t>& out, int x, int y, int width, int height, int format, int type) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&out, x, y, width, height, format, type, this]()->bool {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            _readPixels(out, x, y, width, height, format, type);
            return true;
        }).get();
    }
    void JCLayaGL::_readPixels(std::vector<uint8_t>& out, int x, int y, int width, int height, int format, int type) {
        out.clear();
        ...
        ::glReadPixels(x, y, width, height, format, type, (void*)out.data());
        //glPixelStorei(GL_PACK_ALIGNMENT, 4);
    }
    void JCLayaGL::getFloatv(GLfloat& result, int name) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&result, name]()->bool {
            if (name == GL_DEPTH_CLEAR_VALUE || name == GL_SAMPLE_COVERAGE_VALUE)
            {
                JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            }
    ...
    void JCLayaGL::getIntegerArrayv(std::vector<int>& out, int pname) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&out, pname, this]()->bool {
            _getIntegerArrayv(out, pname);
            return true;
        }).get();
    }
    void JCLayaGL::_getIntegerArrayv(std::vector<int>& out, int pname) {
        int size = 0;
        int* pRet = nullptr;
        ...
        {
            out.push_back(pRet[i]);
        }
    }
    void JCLayaGL::getFloatArrayv(std::vector<float>& out, int name) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&out, name, this]()->bool {
            _getFloatArrayv(out, name);
            return true;
        }).get();
    }
    void JCLayaGL::_getFloatArrayv(std::vector<float>& out, int pname) {
        int size = 0;
        float* pRet = nullptr;
    ...
    void JCLayaGL::getShaderPrecisionFormat(int shaderType, int precisionType, GLint* range, GLint* precision) {
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([shaderType, precisionType, range, precision]()->bool {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
    ...
    {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([cap]()->GLboolean {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
    ...
    {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([target]()->GLenum {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            return ::glCheckFramebufferStatus(target);
        }).get();
    }
    GLint JCLayaGL::getVertexAttrib(GLuint index, GLenum target)
    {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([index, target]()->GLint {
            GLint result = 0;
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
    ...
    GLint JCLayaGL::getTexParameter(GLenum target, GLenum name)
    {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([target, name]()->GLint {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            GLint params;
            ::glGetTexParameteriv(target, name, &params);
            return params;
        }).get();
    }
    GLint JCLayaGL::getRenderbufferParameter(GLenum target, GLenum name)
    {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([target, name]()->GLint {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            GLint params;
            ::glGetRenderbufferParameteriv(target, name, &params);
            return params;
        }).get();
    }
    GLint JCLayaGL::getBufferParameter(GLenum target, GLenum name)
    {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([target, name]()->GLint {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            GLint params;
    ...
    GLuint JCLayaGL::getVertexAttribOffset(GLuint index, GLenum name)
    {
        return JCConch::s_pConchRender->postTaskFromJSToRenderSync([index, name]()->GLuint {
            GLvoid* result;
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            ::glGetVertexAttribPointerv(index, name, &result);
            return *(GLuint*)result;
        }).get();
    }
    void JCLayaGL::getVertexAttribfv(std::vector<float>& out, int index, int target)
    {
        out.clear();
        JCConch::s_pConchRender->postTaskFromJSToRenderSync([&out, index, target]()->bool {
            JCConch::s_pScriptRuntime->dispatchLayaGLBuffer(false);
            float ret[4] = { 0.0 };
