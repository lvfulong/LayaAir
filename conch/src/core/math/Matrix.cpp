...
Matrix::TransformInfo Matrix::extractTransformInfo(const Matrix& matrix) {
    ...
    // 提取旋转（角度）
    info.rotation = Utils::toAngle(atan2(matrix.b, matrix.a));
    ...
    if (info.scaleX != 0 && info.scaleY != 0) {
        ...
        // X方向倾斜角
        info.skewX = Utils::toAngle(atan2(derot_c, derot_d));
        ...
        if (len1 != 0 && len2 != 0) {
            ...
            info.skewY = Utils::toAngle(M_PI / 2 - acos(clampedCosTheta));
        }
    }
...
Matrix& Matrix::transformPoint(const Point& point, Point& out) {
    float x = point.x;
    float y = point.y;
    
}

Matrix& Matrix::transformPointN(const Point& point, Point& out) {
    float x = point.x;
    float y = point.y;
