...
namespace laya
{
    ...
    bool GifLoader::open( const char* p_sFileName,bool p_bInMem )
    {
	    ...
	    m_kIOStream.open( p_sFileName, std::ios::binary );
	    if( !m_kIOStream )
	    {
		    return false;
	    }
    ...
    //------------------------------------------------------------------------------
    unsigned int GifLoader::checkFrames(std::ifstream& p_kIOStream )
    {	
	    unsigned char be;
	    bool fileEnd = false;
	    unsigned int frames=0;
		std::streampos pos = p_kIOStream.tellg();
	    ...
	    {	
		    ...
		    case 0x21:
			    ...
			    case 0xff:
				    ...
				    {	p_kIOStream.read((char*)&be,1);
					    if(be == 0)
						    break;
					    p_kIOStream.seekg(be, std::ios::cur);
				    }
				    break;
			    default:
				    return 0;
		    ...
		    {	
			    ...
			    frames++;
			    p_kIOStream.seekg(8, std::ios::cur);
			    p_kIOStream.read((char*)&bp,1);
			    if((bp&0x80) != 0)
				    lFlag = true;
			    lSize <<= ((bp&0x07)+1);
			    if(lFlag)
				    p_kIOStream.seekg(lSize*3, std::ios::cur);
			    if(!p_kIOStream.good())
				    return 0;
			    p_kIOStream.read((char*)&be,1);
			    while(!p_kIOStream.eof())
    ...
    //------------------------------------------------------------------------------
    bool GifLoader::getAllFrames(std::ifstream& p_kIOStream)
    {	
	    unsigned char be;
	    bool fileEnd = false;
					    ...
					    {	case 0xf9:
							    ...
							    {	m_kIOStream.read((char*)&be,1);
								    ...
								    {	m_kCtrlExt.active = true;
									    ...
									    m_kIOStream.seekg(be, std::ios::cur);
							    }
							    break;
						    case 0xfe:
						    case 0x01:
    ...
    //------------------------------------------------------------------------------
    bool GifLoader::extractData(FRAME* f, std::ifstream& p_kIOStream)
    {	
