...
void Sensor::enableAccelerometer() {
...
void Sensor::disableAccelerometer() {
    if (accelerometer_g_user == nullptr) {
...
void Sensor::disableOrientation() {
    if (orientation_g_user == nullptr) {
...
void Sensor::dispatchAccelerometer(Sensor_Event *event) {
    if (event == nullptr) {
    ...
    laya::DeviceMotionEvent e;
    e.nType = laya::E_DEVICEMOTION;
    e.ax = 0;
    e.ay = 0;
    e.az = 0;
    e.agx = data[0];
    ...
    laya::JCConch::s_pConch->dispatchInputEvent(e);
}

void Sensor::dispatchOrientation(Sensor_Event *event) {
    if (event == nullptr) {
    ...
    laya::DeviceOrientationEvent e;
    e.nType = laya::E_DEVICEORIENTATION;
    e.ra = data[0];
    e.rb = data[1];
    e.rg = data[2];
