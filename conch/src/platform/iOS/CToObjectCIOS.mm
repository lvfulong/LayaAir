...
@end
//-----------------------------------------------------------------
@implementation postCmdEditBoxWXParam
-(id)init:(const char*)defaultValue maxLength:(int)maxLength multiple:(bool)multiple confirmHold:(bool)confirmHold confirmType:(const char*)confirmType prompt:(const char*)prompt promptColor:(const char*)promptColor inputType:(const char*)inputType;
{
    self = [super init];
...
@end
//-----------------------------------------------------------------
@implementation postCmdInt2StringParam
-(id)init:(int)s _p0:(const char*)_iParam0 _p1:(const char*)_iParam1
{
    self = [super init];
    iParam0 = s;
    sBuffer0 = _iParam0;
...
@end
//-----------------------------------------------------------------
@interface CToObjectCIOS ()
@property (nonatomic, strong) CHHapticEngine *haptic_engine;
@end
...
{
    postCmdEditBoxWXParam* _param = (postCmdEditBoxWXParam*)param;
    NSString* defaultValue = [NSString stringWithUTF8String:_param->m_defaultValue.c_str()];
    NSString* confirmType = [NSString stringWithUTF8String:_param->m_confirmType.c_str()];
...
{
    postCmdInt2StringParam* _param = (postCmdInt2StringParam*)param;
    NSString * imgPath = [NSString stringWithUTF8String:_param->sBuffer0.c_str()];
    //[[conchRuntime GetIOSConchRuntime] saveImageToAlbum:imgPath];
}
+(void) postCmdToChooseImage:(id)param
{
    postCmdInt2StringParam* _param = (postCmdInt2StringParam*)param;
    NSString * sizeType = [NSString stringWithUTF8String:_param->sBuffer0.c_str()];
    NSString * sourceType = [NSString stringWithUTF8String:_param->sBuffer1.c_str()];
    [[conchRuntime GetIOSConchRuntime] chooseImage:_param->iParam0 sizeType:sizeType sourceType:sourceType];
}
...
@interface VibrateManager : NSObject
@property (nonatomic, strong) CHHapticEngine *haptic_engine API_AVAILABLE(ios(13.0));
@end

@implementation VibrateManager

static VibrateManager *instance;


+ (VibrateManager *)sharedInstance
{
    static dispatch_once_t oneToken;
    
    dispatch_once(&oneToken, ^{
        instance = [[VibrateManager alloc] init];
    });
    return instance;
    return instance;
}
...
+ (instancetype)alloc {
    NSAssert(!instance, @"VibrateManager类只能初始化一次");
    return [super alloc];
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _haptic_engine = [[CHHapticEngine alloc] initAndReturnError:&error];
        if (!error) {
            [_haptic_engine setAutoShutdownEnabled:true];
        } else {
...
- (void)vibrateWithSeconds:(double)seconds
{
    if (!_haptic_engine) {
        NSLog(@"Could not vibrate using haptic engine");
        return;
    }
    ...
    [[_haptic_engine createPlayerWithPattern:pattern error:&error] startAtTime:0 error:&error];
...
std::string CToObjectCGetCountryCode()
{
    NSString *localeCountryCode = [[NSLocale currentLocale] objectForKey:NSLocaleCountryCode];
    return [localeCountryCode UTF8String];
}
std::string CToObjectCGetTelCountryCode()
{
    NSString *returnstr = @"";
    NSString *localeCountryCode = [[NSLocale currentLocale] objectForKey:NSLocaleCountryCode];
    
    NSString *plistPath = [NSBundle.mainBundle pathForResource:@"attrs" ofType:@"plist"];
    NSDictionary *dictionary = [[NSDictionary alloc] initWithContentsOfFile:plistPath];
    NSArray *keysArray = dictionary[@"country_code"];
    for (int i = 0; i < keysArray.count; i++) {
        NSArray * array = [keysArray[i] componentsSeparatedByString:@","];
        BOOL result = [localeCountryCode isEqualToString:array[0]];
        if(result) {
            returnstr = array[3];
        }

    }
    return [returnstr UTF8String];
}
std::string CToObjectCGetModel()
{
    size_t size;
...
std::string CToObjectCGetLanguage()
{
...
{
    NSString* language = [NSString stringWithUTF8String: p_sLanguage];
    ...
    if ([language isEqualToString:@"zh"])
    {
        ...
        userLanguage = @"en";
    }else if ([language isEqualToString:@"in"]) {
        userLanguage = @"in";
    ...
    LOGI("setLanguage %s", p_sLanguage);
    NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
...
{
    [[VibrateManager sharedInstance] vibrateShort];
}

void CToObjectCVibrateLong()
{
    [[VibrateManager sharedInstance] vibrateLong];
}

void CToObjectCChooseImage(int count, const char* sizeType, const char* sourceType)
{
    postCmdInt2StringParam* param = [[postCmdInt2StringParam alloc] init:count _p0:sizeType _p1:sourceType];
    [CToObjectCIOS performSelectorOnMainThread:@selector(postCmdToChooseImage:) withObject:param waitUntilDone:NO];
}
...
int CToObjectCGetMemoryUsageInByte()
{
...
std::string CToObjectCGetSystemInfo()
{
    ...
    NSDictionary* pDictionary = [NSDictionary dictionaryWithObjectsAndKeys:
                                 [NSString stringWithFormat:@"%@ %@",nsSystemName, nsSystemVersion],@"system",
                                 nsDeviceModel,@"model",
                                 nil];
    NSError* pError = nil;
    ...
    return "{  \"system\" : \"unknow\",  \"model\" : \"ios\" }";
}
...
{
    NSString* msg = [NSString stringWithCString:p_sUrl encoding:NSUTF8StringEncoding];
...
void CToObjectCopy( const char* p_sData )
{
	 ...
	 pasteboard.string = [NSString stringWithCString:p_sData encoding:NSUTF8StringEncoding];
}
...
void CToObjectVideoPlayerGetBitmap(void* obj, laya::BitmapData* bitmap)
{
    ...
    bitmap->reconfigure(imageContainer.width, imageContainer.height, sizeof(int) * 8, laya::ImgType_unknow);
...
void CToObjectCGetSafeAreaInsets(int *top, int *left, int *bottom, int *right) {
    if (strcmp(dispatch_queue_get_label(DISPATCH_CURRENT_QUEUE_LABEL), dispatch_queue_get_label(dispatch_get_main_queue())) == 0)
    {
        ...
        *top = roundf(safeAreaInsets.top * [UIScreen mainScreen].nativeScale);
        *right = roundf(safeAreaInsets.right * [UIScreen mainScreen].nativeScale);
        *bottom = roundf(safeAreaInsets.bottom * [UIScreen mainScreen].nativeScale);
        *left = roundf(safeAreaInsets.left * [UIScreen mainScreen].nativeScale);
    }
    else
    {
        dispatch_sync(dispatch_get_main_queue(), ^{
