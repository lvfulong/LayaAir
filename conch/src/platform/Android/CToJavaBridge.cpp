...
const char* s_javaClasses[] =
{
	...
	"layaair/game/browser/LayaEditBoxNew",
};

const int s_javaClassesSize = 2;
...
CToJavaBridge::CToJavaBridge(){
	...
	m_jMethodGetDisplaySafeArea = NULL;
    m_jClass = NULL;
...
//初始化
    ...
    if (m_jMethodID == NULL)
    {
		...
		m_jMethodGetDisplaySafeArea = p_Env->GetStaticMethodID(m_jClass, "getSafeInsetRect", "()[I");
    }
...
void CToJavaBridge::getJavaByteArray(jbyteArray jba, JNIEnv *env, char **ppData, int& len)
{ 
	if (nullptr == jba) 
	{
        *ppData = nullptr;
    ...
    jbyte* temp = static_cast<jbyte *>(new jbyte[(sizeof(jbyte) * len)]);
	env->GetByteArrayRegion(jba, 0, len, temp);
	*ppData = reinterpret_cast<char *>(temp);
}
...
/** @brief
*  C++直接调用  参数为std::string s1 如saveImageToPhotosAlbum 直接调用通用callMethod
*/
bool CToJavaBridge::callMethod(const char* className, const char* methodName, std::string s1, JavaRet& ret)
{
	std::string param1 = s1;
	replace_all_distinct(param1, "\\", "\\\\");
	replace_all_distinct(param1, "\"", "\\\"");
	std::string result = "[\"" + param1 + "\"]";
	LOGI("saveImageToPhotosAlbum %s", result.c_str());
...
/** @brief
*  C++直接调用  参数为int n, const char*s1, const char*s2,如chooseImage  直接调用通用callMethod
*/
bool CToJavaBridge::callMethod(const char* className, const char* methodName, int n, const char* s1, const char* s2, JavaRet& ret)
{
	std::string param1 = s1;
	replace_all_distinct(param1, "\\", "\\\\");
	replace_all_distinct(param1, "\"", "\\\"");
	...
	char buffer[20];
	sprintf(buffer, "%d", n);
	std::string param13(buffer);
	LOGI("chooseImage %s", param13.c_str());
	LOGI("chooseImage %s", param1.c_str());
	LOGI("chooseImage %s", param2.c_str());
	std::string result = "[" + param13 + ",\"" + param1 + "\",\"" + param2 + "\"]";
	LOGI("chooseImage %s", result.c_str());
	return callMethod(-1, true, className, methodName, result.c_str(), ret);
}
/** @brief
...
bool CToJavaBridge::getClassAndStaticMethod(const char* className, const char* methodName, const char* methodSign, JNIEnv** ppEnv, jclass* pClazz, jmethodID* pMethodId)
{
	ThreadJNIData* pThreadData = checkThreadJNI();
	if (!pThreadData)return false;
	*ppEnv = pThreadData->pThreadJNI;
	...
	*pClazz = thisClass;
	...
	jmethodID methodID = pEnv->GetStaticMethodID(thisClass, methodName, methodSign);
	if (NULL == methodID)
	{
		LOGE("[Debug][???]GetMethodID %s failed", methodName);
		return false;
...
bool CToJavaBridge::getSafeInsetRect(int& left, int& top, int& right, int& bottom)
{
	ThreadJNIData* pThreadData = checkThreadJNI();
	if (!pThreadData)
		return false;
	JNIEnv* pJNI = pThreadData->pThreadJNI;
	jintArray returnArray = (jintArray)pJNI->CallStaticObjectMethod(m_jClass, m_jMethodGetDisplaySafeArea);
    if (!returnArray)//android 8 crash unknow reseaon
        return false;
	jint *arrayBody = pJNI->GetIntArrayElements(returnArray, JNI_FALSE);
	left = arrayBody[0];
	top = arrayBody[1];
	right = arrayBody[2];
	bottom = arrayBody[3];
	pJNI->ReleaseIntArrayElements(returnArray, arrayBody, 0);
	return true;
}
//------------------------------------------------------------------------------
