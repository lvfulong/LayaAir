...
//          ES_WINDOW_POST_SUB_BUFFER_SUPPORTED - specifies that EGL_POST_SUB_BUFFER_NV is supported.
//
GLboolean ESUTIL_API esCreateContext ( ESContext *esContext, GLuint flags )
{
   EGLint configAttribList[] =
   {
       EGL_RED_SIZE,       5,
   ...
   GetClientRect( esContext->hWnd, &wndRect );

   esContext->width = wndRect.right-wndRect.left;
   esContext->height = wndRect.bottom-wndRect.top;
