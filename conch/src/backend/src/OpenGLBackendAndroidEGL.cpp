...
EGLConfig GetConfig(EGLDisplay eglDisplay, bool *out_use_es3) {
  ...
  for (bool try_es3 : std::vector<bool>{true, false}) {
    EGLint config_attribs[] = {EGL_BUFFER_SIZE,
                               ...
                               -1,
                               EGL_RENDERABLE_TYPE,
                               try_es3 ? EGL_OPENGL_ES3_BIT
                                       : EGL_OPENGL_ES2_BIT,
                               EGL_SURFACE_TYPE,
                               EGL_WINDOW_BIT | EGL_PBUFFER_BIT,
                               EGL_NONE};
    EGLint num_configs = 0;
    if (!eglChooseConfig(eglDisplay, config_attribs, nullptr, 0,
                         &num_configs) ||
        num_configs == 0) {
      continue;
    }

    /*CHECK(*/ eglChooseConfig(eglDisplay, config_attribs, &config, 1,
                               &num_configs /*)*/);
    use_es3 = try_es3;
    break;
  }
...
OpenGLBackendAndroidEGL::OpenGLBackendAndroidEGL()
    : m_impl(new OpenGLBackendAndroidEGLImpl()) {}
OpenGLBackendAndroidEGL::~OpenGLBackendAndroidEGL() {
...
void OpenGLBackendAndroidEGL::chooseConfig(const BackendOptions &options) {
  ...
  while (true) {
    const EGLint attribList[] = {EGL_RENDERABLE_TYPE,
                                 ...
                                 EGL_NONE,
                                 EGL_NONE};
    eglChooseConfig(m_impl->m_eglDisplay, attribList, &m_impl->m_EGLConfig, 1,
                    &numConfigs);
    if (numConfigs) {
      LOGI("Config found");
    ...
    if (samples > 0) {
      ...
      EGLBoolean result =
          eglGetConfigs(m_impl->m_eglDisplay, configs, 256, &numTotalConfigs);
      assert(result != EGL_FALSE);
...
void OpenGLBackendAndroidEGL::create(const BackendOptions &options) {
  ...
  EGLBoolean result =
      eglInitialize(m_impl->m_eglDisplay, &majorVersion, &minorVersion);
  assert(result != EGL_FALSE);
  ...
  m_impl->m_EGLContext =
      eglCreateContext(m_impl->m_eglDisplay, m_impl->m_EGLConfig, nullptr,
...
void OpenGLBackendAndroidEGL::makeCurrent() {
  ...
  eglMakeCurrent(m_impl->m_eglDisplay, m_impl->m_EGLSurface,
                 m_impl->m_EGLSurface, m_impl->m_EGLContext);
}
...
void OpenGLBackendAndroidEGL::createScreenSurface(void *nativeHandle) {
  ...
  m_impl->m_aNativeWindow =
      (ANativeWindow *)nativeHandle; // todo ANativeWindow_release
  ...
  EGLint format;
  eglGetConfigAttrib(m_impl->m_eglDisplay, m_impl->m_EGLConfig,
  ...
  m_impl->m_EGLSurface = eglCreateWindowSurface(
      m_impl->m_eglDisplay, m_impl->m_EGLConfig,
      (EGLNativeWindowType)m_impl->m_aNativeWindow, &egl_window_attributes[0]);
  if (m_impl->m_EGLSurface == EGL_NO_SURFACE) {
...
void OpenGLBackendAndroidEGL::onScreenSurfaceResize(int width, int height) {
  ...
  if (m_impl->m_EGLSurface != EGL_NO_SURFACE &&
      (m_impl->m_width != width || m_impl->m_height != height)) {
...
void OpenGLBackendAndroidEGL::preserveBackBuffer() {
  eglSurfaceAttrib(m_impl->m_eglDisplay, m_impl->m_EGLSurface,
