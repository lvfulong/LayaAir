...
std::string Tokenizer::utf8Encode(uint32_t codepoint)
{
    std::string result;
...
uint32_t Tokenizer::parseUnicode()
{
    uint32_t value = 0;
    size_t count = 0;
...
bool Tokenizer::parseEscape(std::string &str)
{
    char c = peek();
...
FontParser::FontParser(std::string_view input)
    : tokenizer_(input), currentToken_(tokenizer_.nextToken()), nextToken_(tokenizer_.nextToken())
{
}
...
const std::unordered_map<std::string, double> FontParser::unitMap = {
    {"cm", 37.8f},        {"mm", 3.78f}, {"in", 96.0f}, {"pt", 96.0f / 72.0f},
    {"pc", 96.0f / 6.0f}, {"em", 16.0f}, {"px", 1.0f}};

void FontParser::advance()
{
    currentToken_ = nextToken_;
    nextToken_ = tokenizer_.nextToken();
}

void FontParser::skipWs()
{
    while (currentToken_.type() == Token::Type::Whitespace)
        advance();
}

bool FontParser::check(Token::Type type) const
{
    return currentToken_.type() == type;
}

bool FontParser::checkWs() const
{
    return nextToken_.type() == Token::Type::Whitespace || nextToken_.type() == Token::Type::EndOfInput;
}

bool FontParser::parseFontStyle(FontProperties &props)
{
...
bool FontParser::parseFontVariant(FontProperties &props)
{
    if (check(Token::Type::Identifier))
    {
        const auto &value = currentToken_.getString();
...
bool FontParser::parseFontWeight(FontProperties &props)
{
...
bool FontParser::parseFontSize(FontProperties &props)
{
    if (!check(Token::Type::Number))
        return false;
...
bool FontParser::parseLineHeight(FontProperties &props)
{
...
bool FontParser::parseFontFamily(FontProperties &props)
{
    while (!check(Token::Type::EndOfInput))
    {
        ...
        while (check(Token::Type::QuotedString) || check(Token::Type::Identifier) || check(Token::Type::Whitespace))
        {
            if (check(Token::Type::Whitespace))
            {
                if (found)
                    trailingWs += currentToken_.getString();
            }
            else
        ...
        if (!found)
            return false; // only whitespace or non-id/string found
...
FontProperties FontParser::parse(const std::string &fontString, bool *success)
{
    FontParser parser(fontString);
    auto result = parser.parseFont();
    if (success)
        *success = !parser.hasError_;
    return result;
}

FontProperties FontParser::parseFont()
{
    ...
    match:
        skipWs(); // success: move to the next non-ws token
    }
