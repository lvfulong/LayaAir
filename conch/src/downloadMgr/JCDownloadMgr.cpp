...
namespace laya{
	static std::map<std::string, std::string> NoHeader;
	#define DEF_OPT_CONNECTTIMEOUT 10
	#define DEF_OPTTIMEOUT 10
...
//#endif
    ...
    void JCDownloadMgr::download(const char *p_pszURL, int p_nPriority,
		...
		std::map<std::string, std::string>& p_vHeaders,      //size()==0则忽略
		...
		if (0 != p_pszURL) {
			...
			IHttpClient* httpClient = m_httpClientManager->createHttpClient(p_pszURL,
				(p_pLocalFile != nullptr ? p_pLocalFile
					: ""), p_ProgCb,
				p_CompleteCb);

			int opttimeout = p_nTimeout == 0 ? DEF_OPTTIMEOUT : p_nTimeout;
			httpClient->setReadTimeout(opttimeout * 1000);
			int connTimeout = p_nConnTimeout == 0 ? DEF_OPT_CONNECTTIMEOUT : p_nConnTimeout;
			httpClient->setConnectTimeout(connTimeout * 1000);
			//LOGI("Download opttimeout  %d %d ", opttimeout, connTimeout);
			for (auto& header : p_vHeaders) {
				httpClient->addHeader(header.first.c_str(), header.second.c_str());
			}
			if (p_pPostData && p_nPostLen > 0) {
				httpClient->setMethod("POST");
				httpClient->postData(p_pPostData, p_nPostLen);
			}
			else {
	...
	void JCDownloadMgr::postData(const char* p_pszURL,const char* p_Buffer, int p_nLength, onEndFunc p_completeCb,
        ...
        if (p_Buffer && p_nLength > 0) {
            httpClient->setMethod("POST");
            httpClient->postData(p_Buffer, p_nLength);
        }
        else {
            httpClient->setMethod("GET");;
        }
...
#else
		m_httpClientManager->cancelAllHttpClients();
#endif
	}
