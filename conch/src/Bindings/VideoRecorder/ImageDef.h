...
public:
	static void AllocNativeImage(NativeImage *pImage)
	{
		...
		switch (pImage->format)
		{
			...
			case IMAGE_FORMAT_I420:
			{
				...
				break;
			default:
				LOGI("NativeImageUtil::AllocNativeImage do not support the format. Format = %d", pImage->format);
				break;
		}
	}
	...
	{
		LOGI("NativeImageUtil::CopyNativeImage src[w,h,format]=[%d, %d, %d], dst[w,h,format]=[%d, %d, %d]", pSrcImg->width, pSrcImg->height, pSrcImg->format, pDstImg->width, pDstImg->height, pDstImg->format);
		LOGI("NativeImageUtil::CopyNativeImage src[line0,line1,line2]=[%d, %d, %d], dst[line0,line1,line2]=[%d, %d, %d]", pSrcImg->pLineSize[0], pSrcImg->pLineSize[1], pSrcImg->pLineSize[2], pDstImg->pLineSize[0], pDstImg->pLineSize[1], pDstImg->pLineSize[2]);
		...
		{
			LOGI("NativeImageUtil::CopyNativeImage invalid params.");
			return;
		}
		...
		switch (pSrcImg->format)
		{
			...
			default:
			{
				LOGI("NativeImageUtil::CopyNativeImage do not support the format. Format = %d", pSrcImg->format);
			}
				break;
		}
