...
class GLCommandEncoder
{
	...
	add_ii_wab(a:number, b:number, arraybuffer:any, length:number, nAlignLength:number, offset?:number):void
    {
        this._need(8 + nAlignLength + 4);
        this.add_ii(a, b);
		this.wab(arraybuffer, length, nAlignLength, offset);
    }
    add_ii_String(a:number, b:number, str:string, len:number):void
    {
	...
	add_iii_wab(a:number, b:number, c:number, arraybuffer:any, length:number, nAlignLength:number, offset?:number):void
    {
        this._need(12 + nAlignLength + 4);
        this.add_iii(a, b, c);
		this.wab(arraybuffer, length, nAlignLength, offset);
    }
    add_iii_String(a:number, b:number, c:number, str:string, len:number):void
    {
	   var ab:ArrayBuffer = conch.strTobufer(str);
       this._need(12 + len + 4);
       this.add_iii(a, b, c);
    ...
    add_iiifi(a:number, b:number, c:number, d:number, e:number):void
    {
        this._need(20);
        var idata:Int32Array = this._idata;
        var i:number = this._idata[0];
    ...
    add_iiii_wab(a:number, b:number, c:number, d:number, arraybuffer:any, length:number, nAlignLength:number, offset?:number):void
    {
	...
	add_iiiiiii_wab(a:number, b:number, c:number, d:number, e:number, f:number, g:number, arraybuffer:any, length:number, nAlignLength:number, offset?:number):void
    {
    ...
    add_iiiiiiii_wab(a:number, b:number, c:number, d:number, e:number, f:number, g:number, h:number, arraybuffer:any, length:number, nAlignLength:number, offset?:number):void
    {
	...
	add_iiiiiiiii_wab(a:number, b:number, c:number, d:number, e:number, f:number, g:number, h:number, j:number, arraybuffer:any, length:number, nAlignLength:number, offset?:number):void
    {
    ...
    getActiveUniformBlockName(program: WebGLProgram, uniformBlockIndex: number):any
    {
        var gl = this._layagl;
        return gl._nativeObj.getActiveUniformBlockName(program ? program.id : 0, uniformBlockIndex);
    }
    getUniformBlockIndex(program: WebGLProgram, uniformBlockName: string):any
    {
        var gl = this._layagl;
    ...
    texImage2D(_args):void
    {
        ...
        if (args.length == 6)
        {
            ...
            else if (args[5]._context && args[5]._context._nativeObj) {//canvas2d
                this.add_iiiiiii(FUNCTION_ID.TEXIMAGE2D, args[0], args[1], args[2], args[3], args[4], args[5]._nativeObj.conchImgId);
            }
            else if (args[5].conchImgId !== undefined)//ImageBitmap
            {
                //函数ID,target, level, internalformat, format, type, image
        ...
        else if (args.length == 9)
        {
            ...
            {
                var ab:ArrayBuffer | ArrayBufferView = args[8];
    ...
    texSubImage2D(_args):void
    {
        ...
        if (args.length == 7)
        {
            ...
            else if (args[6].conchImgId !== undefined)//ImageBitmap 
            {
        ...
        else if (args.length == 9)
        {
            ...
            else if (args[8]._context && args[8]._context._nativeObj ) {//canvas2d
            ...
            else if (args[8].conchImgId !== undefined) //ImageBitmap
            {
...
class LayaGLContext
{
    ...
    public bindBufferRange(target:any, index:number, buffer:WebGLBuffer, offset:number, size:number): void
    {
        this._currentCmdEncoder.bindBufferRange(target, index, buffer, offset, size);
    }
    public bindBufferBase(target:any, index:number, buffer:WebGLBuffer): void
    {
    ...
    public createArrayBufferRef(arrayBuffer:any, type:number, syncRender:boolean):any
    {
        var bufferConchRef:any = conch.createArrayBufferRef(arrayBuffer, type, syncRender, LayaGLContext.ARRAY_BUFFER_REF_REFERENCE);
        arrayBuffer["conchRef"] = bufferConchRef;
        arrayBuffer["_ptrID"] = bufferConchRef.id;
        return bufferConchRef;
    }
}
window["webglPlus"] = conch;//兼容 后续删掉window.webglPlus.createArrayBufferRef
