...
#ifdef USE_FFMPEG
const int AV_TS_MAX_STRING_SIZE = 12;
...
MediaRecorder::MediaRecorder(const char *url, RecorderParam *param) {
	LOGI("MediaRecorder::MediaRecorder url=%s", url);
    strcpy(m_OutUrl, url);
...
int MediaRecorder::StartRecord() {
	LOGI("MediaRecorder::StartRecord");
    int result = 0;
    do {
        /* allocate the output media context */
        avformat_alloc_output_context2(&m_FormatCtx, NULL, NULL, m_OutUrl);
        if (!m_FormatCtx) {
			LOGI("MediaRecorder::StartRecord Could not deduce output format from file extension: using MPEG.\n");
        ...
        if (!(m_OutputFormat->flags & AVFMT_NOFILE)) {
            ...
            if (ret < 0) {
				...
				LOGI("MediaRecorder::StartRecord Could not open '%s': %s", m_OutUrl, av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, ret));
                result = -1;
                break;
            }
        }
        ...
        if (result < 0) {
			char errorBuf[AV_ERROR_MAX_STRING_SIZE]{ 0 };
			LOGI("MediaRecorder::StartRecord Error occurred when opening output file: %s", av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, result));
            result = -1;
            break;
        }
...
int MediaRecorder::OnFrame2Encode(AudioFrame *inputFrame) {
	LOGI("MediaRecorder::OnFrame2Encode inputFrame->data=%p, inputFrame->dataSize=%d", inputFrame->data, inputFrame->dataSize);
    if(m_Exit) return 0;
...
int MediaRecorder::OnFrame2Encode(VideoFrame *inputFrame) {
    if(m_Exit) return 0;
	LOGI("MediaRecorder::OnFrame2Encode [w,h,format]=[%d,%d,%d]", inputFrame->width, inputFrame->height, inputFrame->format);
...
int MediaRecorder::StopRecord() {
	LOGI("MediaRecorder::StopRecord");
    m_Exit = true;
    if(m_pAudioThread != nullptr || m_pVideoThread != nullptr || m_pMediaThread != nullptr) {

		while (!m_VideoFrameQueue.Empty() || !m_AudioFrameQueue.Empty()) {
		...
		char errorBuf[AV_ERROR_MAX_STRING_SIZE]{ 0 };
		LOGI("MediaRecorder::StopRecord while av_write_trailer %s", av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, ret));
...
void MediaRecorder::AddStream(AVOutputStream *ost, AVFormatContext *oc, AVCodec **codec,
    ...
    *codec = avcodec_find_encoder(codec_id);
    if (!(*codec)) {
		LOGI("MediaRecorder::AddStream Could not find encoder for '%s'",
    ...
    if (!ost->m_pStream) {
		LOGI("MediaRecorder::AddStream Could not allocate stream");
        return;
    }
    ost->m_pStream->id = oc->nb_streams-1;
    c = avcodec_alloc_context3(*codec);
    if (!c) {
		LOGI("MediaRecorder::AddStream Could not alloc an encoding context");
        return;
    }
    ost->m_pCodecCtx = c;
    ...
    switch ((*codec)->type) {
        case AVMEDIA_TYPE_AUDIO:
			LOGI("MediaRecorder::AddStream AVMEDIA_TYPE_AUDIO AVCodecID=%d", codec_id);
        ...
        case AVMEDIA_TYPE_VIDEO:
			LOGI("MediaRecorder::AddStream AVMEDIA_TYPE_VIDEO AVCodecID=%d", codec_id);
...
void MediaRecorder::PrintfPacket(AVFormatContext *fmt_ctx, AVPacket *pkt) {
	...
	static char ptsBuffer[AV_TS_MAX_STRING_SIZE]{ 0 };
	static char durationBuffer[AV_TS_MAX_STRING_SIZE]{ 0 };
	static char dtsBuffer[AV_TS_MAX_STRING_SIZE]{ 0 };
	static char ptsTimeBuffer[AV_TS_MAX_STRING_SIZE]{ 0 };
	static char dtsTimeBuffer[AV_TS_MAX_STRING_SIZE]{ 0 };
	static char durationTimeBuffer[AV_TS_MAX_STRING_SIZE]{ 0 };
	LOGI("MediaRecorder::PrintfPacket pts:%s pts_time:%s dts:%s dts_time:%s duration:%s duration_time:%s stream_index:%d",
		av_ts_make_string(ptsBuffer, pkt->pts), av_ts_make_time_string(ptsTimeBuffer, pkt->pts, time_base),
		av_ts_make_string(dtsBuffer, pkt->dts), av_ts_make_time_string(dtsTimeBuffer, pkt->dts, time_base),
		av_ts_make_string(durationBuffer, pkt->duration), av_ts_make_time_string(durationTimeBuffer, pkt->duration, time_base),
           pkt->stream_index);
}

AVFrame *MediaRecorder::AllocAudioFrame(AVSampleFormat sample_fmt, uint64_t channel_layout,
                                        int sample_rate, int nb_samples) {
	LOGI("MediaRecorder::AllocAudioFrame");
    AVFrame *frame = av_frame_alloc();
    int ret;

    if (!frame) {
		LOGI("MediaRecorder::AllocAudioFrame Error allocating an audio frame");
        return nullptr;
    }
    ...
    if (nb_samples) {
        ...
        if (ret < 0) {
			LOGI("MediaRecorder::AllocAudioFrame Error allocating an audio buffer");
            return nullptr;
        }
    }
...
int MediaRecorder::OpenAudio(AVFormatContext *oc, AVCodec *codec, AVOutputStream *ost) {
    ...
    if (ret < 0) {
		char errorBuf[AV_ERROR_MAX_STRING_SIZE]{ 0 };
		LOGI("MediaRecorder::OpenAudio Could not open audio codec: %s", av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, ret));
        return -1;
    }
    ...
    if (ret < 0) {
		LOGI("MediaRecorder::OpenAudio Could not copy the stream parameters");
        return -1;
    }
    ...
    if (!ost->m_pSwrCtx) {
		LOGI("MediaRecorder::OpenAudio Could not allocate resampler context");
        return -1;
    }
    ...
    if ((ret = swr_init(ost->m_pSwrCtx)) < 0) {
		LOGI("MediaRecorder::OpenAudio Failed to initialize the resampling context");
        return -1;
    }
    return 0;
}
...
int MediaRecorder::EncodeAudioFrame(AVOutputStream *ost) {
	LOGI("MediaRecorder::EncodeAudioFrame");
    int result = 0;
    AVCodecContext *c;
    ...
    if (frame) {
        ...
        if (ret < 0) {
			LOGI("MediaRecorder::EncodeAudioFrame Error while av_frame_make_writable");
            result = 1;
            goto EXIT;
        }
		...
		LOGI("MediaRecorder::EncodeAudioFrame dst_nb_samples=%d, frame->nb_samples=%d", dst_nb_samples, frame->nb_samples);
        if (ret < 0) {
			LOGI("MediaRecorder::EncodeAudioFrame Error while converting");
            result = 1;
            goto EXIT;
        }
        frame = ost->m_pFrame;
    ...
    if(ret == AVERROR_EOF) {
		...
		char errorBuf[AV_ERROR_MAX_STRING_SIZE]{ 0 };
		LOGI("MediaRecorder::EncodeAudioFrame audio avcodec_send_frame fail. ret=%s", av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, ret));
        result = 0;
        goto EXIT;
    }
    ...
    while(!ret) {
        ...
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
			...
			char errorBuf[AV_ERROR_MAX_STRING_SIZE]{ 0 };
			LOGI("MediaRecorder::EncodeAudioFrame audio avcodec_receive_packet fail. ret=%s", av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, ret));
            result = 0;
            goto EXIT;
        }
		LOGI("MediaRecorder::EncodeAudioFrame pkt pts=%ld, size=%d", pkt.pts, pkt.size);
        int result = WritePacket(m_FormatCtx, &c->time_base, ost->m_pStream, &pkt);
        if (result < 0) {
			char errorBuf[AV_ERROR_MAX_STRING_SIZE]{ 0 };
			LOGI("MediaRecorder::EncodeAudioFrame audio Error while writing audio frame: %s", av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, ret));
            result = 0;
            goto EXIT;
        }
    }
...
AVFrame *MediaRecorder::AllocVideoFrame(AVPixelFormat pix_fmt, int width, int height) {
	LOGI("MediaRecorder::AllocVideoFrame");
    AVFrame *picture;
    int ret;
    ...
    if (ret < 0) {
		LOGI("MediaRecorder::AllocVideoFrame Could not allocate frame data.");
        return nullptr;
    }
...
int MediaRecorder::OpenVideo(AVFormatContext *oc, AVCodec *codec, AVOutputStream *ost) {
    ...
    if (ret < 0) {
		char errorBuf[AV_ERROR_MAX_STRING_SIZE]{ 0 };
		LOGI("MediaRecorder::OpenVideo Could not open video codec: %s", av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, ret));
        return -1;
    }
    ...
    ost->m_pTmpFrame = av_frame_alloc();
    if (!ost->m_pFrame) {
		LOGI("MediaRecorder::OpenVideo Could not allocate video frame");
        return -1;
    }
    ...
    ret = avcodec_parameters_from_context(ost->m_pStream->codecpar, c);
    if (ret < 0) {
		LOGI("MediaRecorder::OpenVideo Could not copy the stream parameters");
        return -1;
    }
    return 0;
}
...
int MediaRecorder::EncodeVideoFrame(AVOutputStream *ost) {
	LOGI("MediaRecorder::EncodeVideoFrame");
    int result = 0;
    int ret;
    AVCodecContext *c;
    AVFrame *frame;
    ...
    if(videoFrame) {
        ...
        switch (videoFrame->format) {
            ...
            case IMAGE_FORMAT_I420:
                ...
                break;
            default:
				LOGI("MediaRecorder::EncodeVideoFrame unSupport format pImage->format=%d", videoFrame->format);
                break;
        }
    }
    ...
    * internally; make sure we do not overwrite it here */
        ...
        if (srcPixFmt != AV_PIX_FMT_YUV420P) {
            ...
            if (!ost->m_pSwsCtx) {
                ...
                if (!ost->m_pSwsCtx) {
					LOGI("MediaRecorder::EncodeVideoFrame Could not initialize the conversion context\n");
                    result = 1;
                    goto EXIT;
                }
            }
...
//    }
    ...
    if(ret == AVERROR_EOF) {
		...
		char errorBuf[AV_ERROR_MAX_STRING_SIZE]{ 0 };
		LOGI("MediaRecorder::EncodeVideoFrame video avcodec_send_frame fail. ret=%s", av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, ret));
        result = 0;
        goto EXIT;
    }
    ...
    while(!ret) {
        ...
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
			...
			char errorBuf[AV_ERROR_MAX_STRING_SIZE]{ 0 };
			LOGI("MediaRecorder::EncodeVideoFrame video avcodec_receive_packet fail. ret=%s", av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, ret));
            result = 0;
            goto EXIT;
        }
		LOGI("MediaRecorder::EncodeVideoFrame video pkt pts=%ld, size=%d", pkt.pts, pkt.size);
        int result = WritePacket(m_FormatCtx, &c->time_base, ost->m_pStream, &pkt);
        if (result < 0) {
			char errorBuf[AV_ERROR_MAX_STRING_SIZE]{ 0 };
			LOGI("MediaRecorder::EncodeVideoFrame video Error while writing audio frame: %s", av_make_error_string(errorBuf, AV_ERROR_MAX_STRING_SIZE, ret));
            result = 0;
            goto EXIT;
        }
    }
...
void MediaRecorder::StartMediaEncodeThread(MediaRecorder *recorder) {
	LOGI("MediaRecorder::StartMediaEncodeThread start");
    ...
    while (!vOs->m_EncodeEnd || !aOs->m_EncodeEnd) {
		...
		LOGI("MediaRecorder::StartVideoEncodeThread [videoTimestamp, audioTimestamp]=[%lf, %lf]", videoTimestamp, audioTimestamp);
        if (!vOs->m_EncodeEnd &&
	...
	LOGI("MediaRecorder::StartVideoEncodeThread end");
}
#endif // ANDROID