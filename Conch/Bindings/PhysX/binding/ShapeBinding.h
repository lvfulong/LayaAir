...
EMSCRIPTEN_BINDINGS(physx_shape) {
    ...
    class_<PxMaterial>("PxMaterial")
            .function_optional_override("release", optional_override([](PxMaterial &material){
                material.release();
            }))
    ...
    class_<PxShape>("PxShape")
            .function_optional_override("release", optional_override([](PxShape &shape) {
                          if (shape.userData) {
                              free(shape.userData);
        ...
        //     .function("getCapsuleGeometry", &PxShape::getCapsuleGeometry, allow_raw_pointers())
            ...
            .function_optional_override("setMaterials", optional_override([](PxShape &shape, std::vector<PxMaterial *> materials) {
                          return shape.setMaterials(materials.data(), materials.size());
                      }))
            .function_optional_override("setUUID", optional_override([](PxShape &shape, uint32_t uuid) {
                          auto ptr = malloc(sizeof(uint32_t));
                          memcpy(ptr, &uuid, sizeof(uint32_t));
                          shape.userData = ptr;
                      }))
            .function_optional_override("getUUID", optional_override([](PxShape &shape) {
