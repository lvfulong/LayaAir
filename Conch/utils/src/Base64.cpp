...
std::string base64Encode(const char *data, unsigned length,
...
void base64Encode(const char *data, unsigned len, std::vector<char> &out,
                  Base64EncodePolicy policy) {
  out.clear();
  if (!len)
    return;
...
template <typename T>
static inline bool
base64DecodeInternal(const T *data, unsigned length, std::vector<char> &out,
...
bool base64Decode(const std::string &in, std::vector<char> &out,
                  CharacterMatchFunctionPtr shouldIgnoreCharacter,
                  Base64DecodePolicy policy) {
  if (in.empty())
    return base64DecodeInternal<char>(0, 0, out, shouldIgnoreCharacter,
                                               policy);
  // if (in.is8Bit())
  return base64DecodeInternal<char>(in.c_str(), in.length(), out,
