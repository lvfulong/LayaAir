...
namespace laya
{
	...
	{
		int startpos = m_vb->m_buffer2D->needSize(points.size() / 2 * MeshVG::const_stride);//vb的起点。
		...
		int stibid = stib >> 1;	// indexbuffer的起始位置
	...
	{
		int startpos = m_vb->m_buffer2D->needSize(points.size() * MeshVG::const_stride);//vb的起点。
		int f32pos = startpos >> 2;
		float* vbdata = (float*)m_vb->m_buffer2D->getData();
		uint32_t* vbu32Arr = (uint32_t*)m_vb->m_buffer2D->getData();
		int ci = 0;
