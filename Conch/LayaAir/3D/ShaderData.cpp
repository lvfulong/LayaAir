...
namespace laya
{
	...
	void ShaderData::refreshData(int32_t* pData, int nLength, int counts)
	{
		...
		for (int j = 0; j < counts && i < nIntLength; j++)
		{
			...
			switch (nShaderType)
			{
			case laya::ShaderDataType::Number32:
				nSize = 1;
				if (pDataInfo == NULL)
				{
			...
			case laya::ShaderDataType::Matrix4x4:
				...
				i += 16;
				break;
			case laya::ShaderDataType::Number32Array:
				nSize = *(pBufferData + i++);
				if (pDataInfo == NULL)
				{
				...
				break;
			case laya::ShaderDataType::Texture:
				nSize = 1;
				if (pDataInfo == NULL)
				{
					pDataInfo = new ShaderData::DataInfo(nShaderType, nSize, sizeof(int));
				...
				memcpy(pDataInfo->data, pBufferData + i++, sizeof(int));
				break;
			case laya::ShaderDataType::ShaderDefine:
				assert(true);
				break;
			case laya::ShaderDataType::UBO:
				nSize = 1;
				if (pDataInfo == NULL)
				{
					pDataInfo = new ShaderData::DataInfo(nShaderType, nSize, sizeof(int));
